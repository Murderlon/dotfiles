...
-- - Press `<Leader>` and briefly wait. A window with information about next
--   available keys should appear.
...
later(function()
  ...
  miniclue.setup({
    ...
    -- Show clue window quickly after a trigger key is pressed
    window = { delay = 200 },
...
-- for quick overview and navigation. Opened by default. Example usage:
...
later(function()
  ...
  -- Override Normal mode `gr` from mini.operators replace operator with LSP
  -- references. Visual/operator-pending replace mappings remain available.
