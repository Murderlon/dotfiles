...
-- Automatically reload saved Lua config files. This is intentionally defined in
-- 'init.lua' with its own clearing group to avoid duplicate autocommands after
-- this file itself is reloaded.
vim.api.nvim_create_autocmd('BufWritePost', {
  group = vim.api.nvim_create_augroup('config-auto-reload', { clear = true }),
  pattern = vim.fn.stdpath('config') .. '/**/*.lua',
  callback = function(args)
    local ok, err = pcall(dofile, args.file)
    if ok then
      vim.notify('Reloaded ' .. vim.fn.fnamemodify(args.file, ':t'))
    else
      vim.notify(err, vim.log.levels.ERROR, { title = 'Config reload failed' })
    end
  end,
  desc = 'Reload saved Lua config file',
})
